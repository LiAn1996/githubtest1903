...
int process_admin_adduser_request(int acceptfd,MSG *msg)   //管理员添加
{
	printf("------------%s-----------%d.\n",__func__,__LINE__);
	printf("------------%s-----------%d.\n",__func__,__LINE__);
	char *errmsg = NULL;
...
int process_admin_query_request(int acceptfd,MSG *msg)     //管理员查询
{
	printf("------------%s-----------%d.\n",__func__,__LINE__);
	char sql[128];
	ssize_t recvbytes;
...
int process_admin_history_request(int acceptfd,MSG *msg)   //查询历史记录
{
	printf("------------%s-----------%d.\n",__func__,__LINE__);
	char sql[DATALEN];
	ssize_t recvbytes;
...
int process_client_quit_request(int acceptfd,MSG *msg)      //退出
{
	printf("------------%s-----------%d.\n",__func__,__LINE__);
	//历史记录
...
{
	int sockfd;
	int acceptfd;   /* 定义一个服务器socket文件描述符*/
	ssize_t recvbytes;  //不知道是啥数据类型 干啥的？
...
//操作数据库
	if(sqlite3_open(STAFF_DATABASE,&db) != SQLITE_OK)    //打开数据库函数  成功返回 0（SQLITE_OK）  失败返回-1
	{
		printf("%s\n",sqlite3_errmsg(db));
	}else
	{
...
//填充网络结构体  
	...
	serveraddr.sin_port = htons(atoi(argv[2]));   //端口注意字节序转换
	serveraddr.sin_addr.s_addr = inet_addr(argv[1]); //IP地址要转换成32位网络地址
 ...
 * 由此来通知执行了select()的进程哪个句柄可读。
 ...
  系统提供了FD_SET(将fd加入set集合), FD_CLR（将fd从set中清除）,
  FD_ISSET（检测fd是否在set集合中，不再则返回0）, FD_ZERO（将set清零，使set中不含任何fd），进行操作 */

//定义一张表
	fd_set readfds,tempfds;  //表示定义两个套接字的组合
//清空表
	FD_ZERO(&readfds);
	FD_ZERO(&tempfds);
//添加要监听的事件
...
#if 0   //添加线程控制部分
	pthread_t thread[N];  // 用途：pthread_t用于声明线程ID
	int tid = 0;
#endif

	while(1)
	{
		tempfds = readfds;  //记得重新添加
		retval = select(nfds+1,&tempfds,NULL,NULL,NULL);
		/*	在编程的过程中，经常会遇到许多阻塞的函数，
		...
		for(i = 0;i<nfds+1;i++)
		{
			...
			{	
				...
				{ 
					...
					FD_SET(acceptfd,&readfds);   //添加接受的事件
					nfds = nfds > acceptfd ? nfds : acceptfd;  //三目运算 给nfds赋值
				}else
				{
					//收发数据  recv/send
